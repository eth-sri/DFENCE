...
namespace llvm
{
	...
	class ExecutionEngine
	{
		...
		protected:
		int counter;
		// make a virtual address alligned, as is needed by malloc from mmap
		// allignment is 4K
		void* makeAddressAlligned(void*);
#if defined(VIRTUALMEMORY)
		// nativeToVirtual maps native addresses(from the native malloc) to virtual addresses(generated by our malloc)
		std::map<void*, void*> nativeToVirtual;
		// virtualToNative maps virtual addresses to native ones 
		std::map<void*, void*> virtualToNative;

		// this is the next virtual address to be generated
		...
		// this map is to tell how many bytes are allocated at every virtual base address - global on the heap
		std::map<void*, int> bytesAtVirtualAddress;

		// debugging
		//std::vector< std::pair<int,int> > globalAddresses;
#else
		std::map<void*, int> bytesAtPhysicalAddress;
#endif
...
#if defined(VIRTUALMEMORY)
		// this function remaps all Global Variables to virtual addresses
		// must be called in the begining before the start of the interpreter itself
		void virtualizeGlobalVariables();

		// this function get Native address if it is given Virtual
		// this is for use outside ExecutionEngine and Interpreter
		//void *getNativeAddress(void *);

		// this function given a virtual address return the virtual base address
		// at which memory the address is pointing to.
		// only GlobalVariable addresses and malloced memory is considered
		// if we cannot map the address to base address, for example,
		// because of out-of-bounds error or address to a memory on the stack
		// we just return NULL
		void *getVirtualBaseAddressHeap(void *);
#else
		void physicalizeGlobalVariables();
		void *getPhysicalBaseAddressHeap(void *);
#endif
	...
	namespace EngineKind
	{
		...
		const static Kind Either = (Kind)(JIT | Interpreter | Fence);
	}
